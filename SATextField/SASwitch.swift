...
class SASwitch: UIView {
    ...
    internal weak var delegate: SASwitchDelegate?
    ...
    fileprivate init(_ onColor: UIColor, _ offColor: UIColor, _ dotColor: UIColor, _ isOn: Bool, _ myFrame: CGRect, _ dotSpace: Int, _ smallDotMultiplier: CGFloat, _ dotTravelTime: TimeInterval) {
        ...
        if isOn {
          //  super.init(frame: CGRect(x: dotSpacer, y: dotSpacer, width: ballDiameter, height: ballDiameter))
           super.init(frame: CGRect(x: myFrame.width - ballDiameter - dotSpacer, y: dotSpacer, width: ballDiameter, height: ballDiameter)) // Original
          ...
          //  super.init(frame: CGRect(x: myFrame.width - ballDiameter - dotSpacer, y: dotSpacer, width: ballDiameter, height: ballDiameter))
           super.init(frame: CGRect(x: dotSpacer, y: dotSpacer, width: ballDiameter, height: ballDiameter)) // Original
        ...
        NSLayoutConstraint(item: centerBall ?? UIView(), attribute: .width, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1, constant: centerBallDiameter).isActive = true
        NSLayoutConstraint(item: centerBall ?? UIView(), attribute: .height, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1, constant: centerBallDiameter).isActive = true
        NSLayoutConstraint(item: centerBall ?? UIView(), attribute: .centerX, relatedBy: .equal, toItem: self, attribute: .centerX, multiplier: 1, constant: 0).isActive = true
        NSLayoutConstraint(item: centerBall ?? UIView(), attribute: .centerY, relatedBy: .equal, toItem: self, attribute: .centerY, multiplier: 1, constant: 0).isActive = true
        if isOn {
...
protocol SASwitchDelegate: AnyObject {
